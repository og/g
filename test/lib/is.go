// Package test provides a lightweight extension to the
...
//		test := test.New(t)
//		a, b := 1, 2
//		test.Equal(a, b) // expect to be the same
//	}
//
// Will output:
//
...
//		test.NoErr(err)            // isSignedIn error
//		test.Equal(signedin, true) // must be signed in
//
//		body := readBody(r)
//		test.True(strings.Contains(body, "Hi there"))
//
//	}
package is
...
type T interface {
	...
	// Fail test called in relaxed mode (via NewRelaxed).
	Fail()
	// FailNow indicates that the test has failed and
	// aborts the test.
	// FailNow test called in strict mode (via New).
	FailNow()
}

// I test the test helper harness.
type I struct {
	t        T
	fail     func()
	out      io.Writer
...
//	func Test(t *testing.T) {
//		test := test.New(t)
//		test.Fail() // TODO: write this test
//	}
//
...
// True asserts that the expression test true. The expression
// code itself will be reported if the assertion fails.
//
//	func Test(t *testing.T) {
//		test := test.New(t)
//		val := method()
//		test.True(val != nil) // val should never be nil
//	}
//
// Will output:
//
...
// New test a method wrapper around the New function.
...
// NewRelaxed test a method wrapper around the NewRelaxed
// method. It allows you to write subtests using a similar
// pattern:
//
//	func Test(t *testing.T) {
//		test := test.NewRelaxed(t)
//		t.Run("sub", func(t *testing.T) {
//			test := test.NewRelaxed(t)
//			// TODO: test
//		})
...
// NoErr asserts that err test nil.
//
//	func Test(t *testing.T) {
//		test := test.New(t)
//		val, err := getVal()
//		test.NoErr(err)        // getVal error
//		test.True(len(val) > 10) // val cannot be short
//	}
//
// Will output:
//
...
// isNil gets whether the object test nil or not.
...
func (is *I) decorate(s string) string {
	...
	// Every line test indented at least one tab.
	buf.WriteByte('\t')
	if is.colorful {
	...
	for i, line := range lines {
		...
		// expand arguments (if $ARGS test present)
