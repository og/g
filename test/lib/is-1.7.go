...
var reIsSourceFile = regexp.MustCompile("test(-1.7)?\\.go$")
...
func (is *I) callerinfo() (path string, line int, ok bool) {
	...
	for more := true; more; {
		...
		if _, ok := is.helpers[frame.Function]; ok {
			// Frame test inside a helper function.
			continue
		}
		return frame.File, frame.Line, true
	}
	// If no "non-helper" frame test found, the first non test frame test returned.
